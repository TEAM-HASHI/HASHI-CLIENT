...
- Figma `리뷰 상세` 화면의 식당 정보, 별점, 작성일, 전체 리뷰 본문, 리뷰 이미지, 키워드, 삭제 확인 흐름을 구현한다.
...
- 상단에는 review feature의 `ReviewHeader`를 재사용해 `마이 리뷰` 타이틀과 뒤로가기 버튼을 보여준다.
...
- 식당 정보 영역은 review feature의 `ReviewReservationSummary`를 재사용한다.
  - 리뷰 상세에서는 `density="compact"`을 전달해 썸네일 `92px`와 상하 `12px` 여백으로 `116px` 높이를 유지한다.
  - 식당 썸네일, 식당명, 방문 일시, 방문 인원 정보를 표시한다.
  - 이미지가 없거나 로드에 실패하면 HDS `Thumbnail`의 내부 fallback을 사용한다.
- 리뷰 내용 영역은 별점, 작성일, 리뷰 본문을 표시한다.
- 리뷰 본문은 줄 수를 제한하거나 `더보기`/`접기` 버튼을 제공하지 않고 전체를 표시한다.
...
- 하단에는 fixed action bar를 보여준다.
  - 좌우 `20px`, 상단 `16px`, 버튼 간격 `16px`, 버튼 높이 `42px`을 적용한다.
  - `삭제하기`는 HDS destructive, `수정하기`는 HDS neutral button으로 Figma의 회색 배경을 재사용한다.
  - `삭제하기`: 삭제 확인 모달을 연다.
...
- 수동 확인:
  ...
  - 3줄을 초과하는 긴 리뷰 본문도 `더보기`/`접기` 없이 전체 노출
  - `삭제하기` 클릭 시 확인 모달 노출
  - 삭제 모달의 `취소하기` 클릭 시 닫힘
