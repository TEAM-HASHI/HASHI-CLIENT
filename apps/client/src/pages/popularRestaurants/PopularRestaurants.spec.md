...
- route module: `apps/client/src/app/routes/popular.tsx`
- layout: `RootLayout`
- access type: public
- guard: none
...
- route registration:
  ...
  - `apps/client/src/app/routes/popular.tsx`
...
- build-time loader는 기본 필터(`type=popular`, `genre=all`, `sort=basic`, `size=10`)의 첫 페이지를 필수 데이터로 prefetch합니다.
- clientLoader의 prefetch 실패는 빈 hydration state로 복구하여 Header와 FilterBar를 유지하는 기존 local error UI가 처리합니다.
- HTML에는 인기 맛집 목록 title, description, canonical, Open Graph metadata를 포함합니다.
