...
- 알려진 keyword `code`를 review feature의 HDS icon과 매핑하며, code가 없을 때는 정식 API label로 아이콘을 보완한다.
...
| 리뷰 작성            | `POST /api/v1/reviews`                           | `reservationId`, `rating`, `keywordCodes`, `content`, `imageFileKeys` | `/reviews/:reviewId` 이동, 작성 context/내 리뷰 개수/내 리뷰 목록/식당 상세/작성 가능 예약 cache 무효화 | 현재 화면 유지 및 오류 표시   |
