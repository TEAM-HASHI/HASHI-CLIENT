...
HDS에서는 **native input 렌더링, 검색 아이콘 고정 배치, 입력 영역 layout, placeholder/value 전달, disabled 상태, focus-visible/focus-within, 기본 접근성 계약만 담당**합니다. 실제 검색 실행, debounce, query parameter 동기화, API 호출, 최근 검색어, 자동완성, analytics, 도메인 copy/data 구성은 각 App Shell / Page / Feature에서 처리합니다.
...
- default: 기본 입력 가능 상태
- focused: input 또는 root 내부에 focus가 있는 상태
...
- color: icon은 검색 필드 기본 톤을 따릅니다.
- disabled: opacity로 최소 시각 구분을 제공합니다.
- focus-within: keyboard focus outline 또는 ring을 제공합니다.
...
- focus-visible/focus-within: keyboard focus가 확인 가능해야 합니다.
