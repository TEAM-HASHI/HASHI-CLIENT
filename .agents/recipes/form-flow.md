...
4. React submit event 타입은 deprecated 경고가 나는 `FormEvent`를 직접 import하지 않고 `SyntheticEvent<HTMLFormElement>` 또는 JSX 문맥 타입 추론을 사용합니다.
5. validation, disabled, loading, error, success 상태를 분리합니다.
6. API mutation이 있으면 optimistic update, invalidation, error reset 책임을 명확히 합니다.
7. 접근성 속성과 keyboard submit 동작을 확인합니다.
