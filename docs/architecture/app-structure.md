...
```text
src/
  root.tsx          document shell과 전역 Provider 조립
  entry.client.tsx  browser hydration entry
  routes.ts         React Router Framework Mode route config
  app/
    layout/         route layout UI
    providers/      전역 Provider 조립
    router/         path 상수와 access guard
    routes/         route module, loader, metadata, resource route
```
...
- Framework Mode route config는 `apps/client/src/routes.ts`에서 관리합니다.
- 각 route의 page 조립, loader, metadata는 `apps/client/src/app/routes`의 route module이 소유합니다.
- `apps/client/src/app/router`는 route path 상수와 access guard를 관리합니다.
- route path는 문자열을 흩뿌리지 않고 상수화를 검토합니다.
- URL params와 search params는 사용하는 위치에서 명시적으로 읽고 검증합니다.
- route module 분할은 Framework Mode의 route boundary를 따릅니다.
