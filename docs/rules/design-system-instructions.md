...
Overlay, calendar, date/time input처럼 접근성 상호작용이 복잡한 HDS primitive는 `react-aria-components`를 기반으로 구현할 수 있습니다.
...
- 디자인 구현 전 `@hashi/hds-ui`, `@hashi/hds-icons`에 재사용 가능한 컴포넌트와 아이콘이 있는지 먼저 확인합니다.
...
- 공통 Figma에 존재하더라도 제품 의미가 강한 UI는 바로 `packages/hds-ui`로 승격하지 않습니다.
...
1. `packages/hds-ui`의 기존 컴포넌트를 사용합니다.
2. `packages/hds-icons`의 기존 아이콘을 사용합니다.
3. 제품 의미가 없는 반복 UI라면 `packages/hds-ui`에 컴포넌트 추가를 검토합니다.
4. 앱 전용이지만 여러 화면에서 반복된다면 app 내부 shared component로 작성합니다.
5. 한 페이지나 한 feature에서만 쓰는 UI라면 page-local component로 작성합니다.

| 대상                         | 위치                            | 기준                                            |
| ---------------------------- | ------------------------------- | ----------------------------------------------- |
| HDS UI primitive             | `packages/hds-ui`               | 제품 의미가 없고 재사용 가능한 public component |
| HDS icon                     | `packages/hds-icons`            | 제품 의미가 없고 재사용 가능한 icon component   |
...
- Figma 구현 시 기존 HDS 컴포넌트와 아이콘 매핑을 먼저 확인합니다.
- Figma Code Connect를 도입한 뒤에는 Code Connect 매핑을 우선합니다.
- Code Connect 매핑이 없으면 기존 HDS 컴포넌트, app shared component, Figma variant 구조를 비교해 최소 구현 범위를 정합니다.
- Code Connect 매핑이 없다는 이유만으로 바로 새 HDS 컴포넌트를 만들지 않습니다.
- Figma의 variant, size, tone, state는 명시적인 component props로 표현합니다.
- Figma의 화면별 copy, 데이터 예시, API 흐름은 HDS props나 내부 로직으로 고정하지 않습니다.
...
- foundation token은 `@hashi/hds-tokens/styles.css`의 Tailwind v4 `@theme` 토큰과 기존 문서 값을 기준으로 구현합니다.
...
- 아이콘은 `@hashi/hds-icons`를 우선 사용합니다.
- 새 아이콘은 재사용 가능하고 product-agnostic한 경우에만 `packages/hds-icons`에 추가합니다.
...
## HDS Component Implementation

- 새 HDS 컴포넌트는 먼저 [Design System Component Plan](../architecture/design-system-component-plan.md)의 승격 기준을 통과해야 합니다.
- scaffolding이 필요한 경우 `pnpm gen:ds-component` 사용을 우선 검토합니다.
- 컴포넌트는 `packages/hds-ui/src/components/{componentFolder}` 아래에 둡니다.
...
- `react-aria-components`를 사용하는 경우 HDS public API가 라이브러리 prop 이름에 불필요하게 종속되지 않도록 감싸고, 스타일과 도메인 비의존 public contract를 HDS에서 유지합니다.
...
Storybook story를 추가하거나 수정하면 `pnpm build-storybook` 영향 범위를 확인합니다.

## Review Checklist

디자인시스템 구현 후 다음 항목을 확인합니다.

- 기존 HDS 컴포넌트나 아이콘을 재사용할 수 있었는가?
- app 전용 로직이 `packages/hds-ui`에 들어가지 않았는가?
