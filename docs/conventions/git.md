...
SIKSA Client는 Jira로 작업을 관리하고 GitHub로 코드 리뷰와 병합을 관리합니다.
...
- Jira key는 `SIKSA-번호` 형식을 유지합니다.
- 작업 요약은 영어 소문자와 하이픈을 사용합니다.

```text
init/SIKSA-1-project-setting
feat/SIKSA-12-login
fix/SIKSA-18-button-style
refactor/SIKSA-24-auth-logic
docs/SIKSA-30-convention-docs
chore/SIKSA-31-package-setting
```
...
```bash
git checkout -b feat/SIKSA-12-login
```
...
```bash
git checkout feat/SIKSA-12-login
git fetch origin
...
```text
docs(docs): SIKSA-30 컨벤션 문서 추가
feat(apps/client): SIKSA-12 로그인 페이지 구현
fix(packages/sds-ui): SIKSA-18 Button disabled 스타일 수정
chore(root): SIKSA-31 pnpm workspace 설정 수정
```
...
```bash
git push --force-with-lease origin feat/SIKSA-12-login
```
