...
```bash
pnpm --filter @hashi/client add -D package-name
```

HDS UI 패키지 dependency:

```bash
pnpm --filter @hashi/hds-ui add package-name
```

HDS Icons 패키지 dependency:

```bash
pnpm --filter @hashi/hds-icons add package-name
```
...
```bash
pnpm --filter @hashi/client add @hashi/hds-icons@workspace:*
```

catalog에 새 외부 패키지를 등록하면서 추가할 때:

```bash
pnpm --filter @hashi/client add package-name --save-catalog
```
