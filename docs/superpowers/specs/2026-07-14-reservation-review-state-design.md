...
- 삭제된 리뷰는 `리뷰가 삭제된 예약입니다` 문구만 비활성 상태로 표시한다.
...
| `DELETED`     | `reviewStatus === 'DELETED'`                                                       | `리뷰가 삭제된 예약입니다` 문구만 표시하고 클릭 이벤트를 연결하지 않는다. |
...
- 삭제된 리뷰 카드에 `리뷰가 삭제된 예약입니다`가 표시되는지 확인한다.
...
2. `reviewStatus: DELETED`인 카드에는 `리뷰가 삭제된 예약입니다`가 비활성 상태로 표시된다.
