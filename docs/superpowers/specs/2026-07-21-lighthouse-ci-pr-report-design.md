...
상세 Lighthouse HTML/JSON 결과는 GitHub Actions Artifact로 보관한다. 리뷰어가 PR 화면을 벗어나지 않고 핵심 결과를 확인할 수 있도록 PR 코멘트에는 category, metric, resource를 영역으로 구분한 단일 세로 요약표와 개선 audit 최대 3개의 우선순위 목록을 표시한다.
...
- PR에는 대표 실행의 category 점수·상태, 핵심 성능 지표, resource 크기를 단일 세로 요약표로 표시하고 개선 audit 최대 3개를 순서 있는 목록으로 생성하거나 갱신한다.
...
- category 점수, FCP, LCP, CLS, TBT, Script, Image 값을 `영역 | 항목 | 결과` 단일 Markdown 표로 생성한다.
- 실행 가능한 실패 audit을 절감 시간과 byte 기준으로 정렬해 개선점 최대 3개를 순서 있는 목록으로 표시한다.
...
9. PR 코멘트에는 대표 category, metric, resource 결과를 단일 세로 요약표로 표시하고 개선 audit 최대 3개를 순서 있는 목록으로 표시한다.
...
- Node 내장 test runner로 점수 반올림, 상태 색상, representative run 선택, 핵심 지표 단위 변환, 단일 세로 요약표, 개선점 우선순위와 순서 있는 목록, 기존 코멘트 update와 신규 create를 검증한다.
...
7. PR 코멘트에 네 category 점수·상태, FCP/LCP/CLS/TBT, Script/Image가 단일 세로 요약표로 표시되고 개선 audit 최대 3개가 순서 있는 목록으로 표시된다.
