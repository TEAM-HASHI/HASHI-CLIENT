...
상세 Lighthouse HTML/JSON 결과는 GitHub Actions Artifact로 보관하고, 리뷰어가 PR 화면을 벗어나지 않고 대표 점수를 확인할 수 있도록 PR 코멘트에 점수 표와 Mermaid 막대그래프를 표시한다.
...
- PR에는 대표 실행 점수 표와 Mermaid 막대그래프를 하나의 고정 코멘트로 생성하거나 갱신한다.
...
- Markdown 점수 표, Mermaid `xychart`, workflow run 링크를 생성한다.
...
9. PR 코멘트에는 대표 점수 표, Mermaid 차트, 해당 workflow run 링크를 표시한다.
...
- Node 내장 test runner로 점수 반올림, 상태 색상, representative run 선택, Markdown 생성, 기존 코멘트 update와 신규 create를 검증한다.
...
7. PR 코멘트에 네 category 점수 표와 Mermaid 막대그래프가 표시된다.
