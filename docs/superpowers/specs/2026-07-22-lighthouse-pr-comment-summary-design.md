...
기존 Lighthouse CI PR 코멘트는 대표 실행의 category 점수 표, Mermaid 막대그래프, GitHub Actions 실행 링크, 측정 시각을 표시했다. 첫 요약안에서 그래프와 링크를 제거하고 category 표와 핵심 지표 표를 분리했지만, 서로 다른 표 폭과 여섯 열의 가로 지표 표가 시각적으로 어색해 최종 C안에서는 단일 세로 요약표로 통합한다.
...
- PR 코멘트에서 category 결과, 핵심 성능 지표, resource 크기를 폭이 일정한 하나의 표로 확인할 수 있게 한다.
...
2. `## 🔦 Lighthouse 결과` 제목
3. `영역`, `항목`, `결과` 열을 가진 단일 세로 요약표
4. `### 💡 개선 우선순위`와 최대 3개의 순서 있는 audit 권고

단일 표에서 category는 `Score`, FCP/LCP/CLS/TBT는 `Metric`, Script/Image는 `Resource` 영역으로 구분한다. Score 결과는 상태 emoji와 굵은 점수를 함께 표시하고, Metric과 Resource 결과는 inline code로 표시한다. Mermaid 그래프, GitHub Actions 실행 링크, 대표 실행 시각은 표시하지 않는다.
...
후보는 예상 시간 절감, 예상 byte 절감, 개선 기회 여부, 점수 부족 폭 순으로 우선순위를 정하고 상위 3개만 순서 있는 목록으로 표시한다. 각 항목은 Lighthouse report가 제공하는 audit 제목을 사용하며, 절감 시간 또는 byte 정보가 있으면 함께 표시한다. 후보가 없으면 `감지된 주요 개선점이 없습니다.`를 순서 있는 한 항목으로 표시한다.
...
- category, metric, resource를 구분한 `영역 | 항목 | 결과` 단일 세로 요약표를 생성한다.
- 시간, CLS, resource size를 지정된 단위로 변환한다.
- 개선 후보를 우선순위에 따라 최대 3개의 순서 있는 목록으로 표시한다.
- 개선 후보가 없을 때 fallback 문구를 표시한다.
- 이전 `Category | Score | Status` 표와 여섯 열 핵심 지표 표를 생성하지 않는다.
- Mermaid, Actions 실행 링크, 생성 시각을 포함하지 않는다.
- 기존 코멘트 update, 신규 create, 오류 시 skip 동작을 보존한다.

## Documentation Impact

기존 Lighthouse CI 설계 문서와 PR checklist에서 PR 코멘트 설명을 단일 세로 요약표와 개선 우선순위 목록으로 갱신한다. Artifact 보관, API smoke check, Lighthouse 측정 및 warning 정책은 변경하지 않는다.

## Acceptance Criteria

1. PR 코멘트에 category, metric, resource 결과가 `영역 | 항목 | 결과` 단일 세로 요약표로 표시된다.
2. Score는 상태 emoji와 굵은 점수로, FCP/LCP/CLS/TBT와 Script/Image는 단위가 포함된 inline code로 표시된다.
3. 개선 여지가 큰 audit이 최대 3개의 순서 있는 목록으로 표시되며, 후보가 없으면 명시적인 fallback 문구가 표시된다.
